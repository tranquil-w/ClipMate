...
/// <summary>
/// 设置视图模型基类，管理应用程序的所有用户可配置选项
/// </summary>
/// <remarks>
/// 负责处理主题切换、快捷键设置、窗口位置、开机自启动、
/// 静默启动、管理员权限和日志级别等设置项的读取和保存。
/// </remarks>
public partial class SettingsViewModelBase : ObservableObject
{
    ...
    private void StartHotkeyRecording(HotkeyRecordingTarget target)
    {
        ...
        _recordingHandler = (_, e) =>
        {
            ...
            _ = _uiDispatcher.InvokeAsync(() => OnHotkeyRecorded(descriptor.DisplayString, recordingTarget));
        };
        ...
        try
        {
            switch (target)
            {
                case HotkeyRecordingTarget.MainWindow:
                    ...
                    _uiDispatcher.Invoke(RegisterHotkeyFromSettings);
                    break;
    ...
    {
        UpdateSelectedLogLevelOption(value);
        if (_isInitializing)
        {
            return;
        }
    ...
    partial void OnSelectedLogLevelOptionChanged(LogLevelOption? value)
    {
        ...
        if (SelectedLogLevel != value.Level)
        {
        ...
        Func<Task>? additionalWorkAsync = null)
    {
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                if (uiCallback != null)
                {
                    _uiDispatcher.Invoke(uiCallback);
                }
            }
            catch (Exception ex)
            {
    ...
    private void UpdateSelectedLogLevelOption(LogEventLevel value)
    {
        var option = LogLevels.FirstOrDefault(item => item.Level == value);
        if (!Equals(SelectedLogLevelOption, option))
        {
            SelectedLogLevelOption = option;
        }
    }
    ...
    private string BuildAutoStartMethodText()
    {
        ...
        {
            AutoStartMethod.TaskScheduler => "自启动方式：任务计划程序（管理员）",
            AutoStartMethod.Registry => "自启动方式：注册表（当前用户）",
            _ => "自启动方式：未启用"
        };
    }
    ...
    private void RegisterHotkeyFromSettings()
    {
        try
        {
            ...
            {
                // 其他快捷键使用 NHotkey 处理
                var success = _hotkeyService.RegisterMainWindowToggleHotkey(_applicationService.ToggleMainWindow);
                if (success)
                {
